...
import { 
  ...
  Globe, MoreHorizontal, User as UserIcon
} from 'lucide-react';
...
// Global cache for user profiles to optimize DB calls
...
const UserAvatar: React.FC<{ userId: string; className?: string; onClick?: () => void }> = ({ userId, className, onClick }) => {
  ...
  useEffect(() => {
    ...
    // Optimization: Single fetch per component mount, cached globally
  ...
  return (
    <div 
      className={`relative rounded-full p-[2px] ${border?.class} ${className || 'w-10 h-10'} cursor-pointer flex-shrink-0 transition-transform hover:scale-105`}
      onClick={onClick}
      style={user?.profileBorder === 'custom' && user.customColor ? { boxShadow: `0 0 10px ${user.customColor}`, borderColor: user.customColor, borderStyle: 'solid', borderWidth: '2px' } : {}}
    >
      <img 
...
export default function App() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else if (err.code === 'auth/invalid-email') alert('Invalid email or username format.');
  ...
  const handleUploadSubmit = async (formData: Partial<Item>) => {
    ...
    if (userData?.muted) return alert("You are muted and cannot upload.");
    ...
    if (isEdit) {
      ...
      if (!canEditItem(userData, originalItem.authorId)) return alert("Permission denied.");
  ...
  const handleDeleteItem = async (id: string, authorId: string) => {
    if (!canEditItem(userData, authorId)) return alert("Permission denied.");
  ...
  const handleRating = async (itemId: string, ratingVal: number, review: string) => {
    ...
    // Prevent author from rating their own item
    const item = items.find(i => i.id === itemId);
    if (item && item.authorId === currentUser.uid) {
        return alert("You cannot rate your own creation.");
    }
