...
const UserAvatar: React.FC<{ userId: string; className?: string; onClick?: () => void; editable?: boolean }> = ({ userId, className, onClick, editable = false }) => {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setUploading(true);
    try {
      // MENGGUNAKAN FUNGSI GLOBAL uploadToImgBB
      ...
      const updatedUser = { ...user, profilePic: imageUrl } as User;
      setUser(updatedUser);
  ...
  return (
    <div 
      className={`relative rounded-full p-[2px] ${border?.class} ${className || 'w-10 h-10'} ${editable || onClick ? 'cursor-pointer' : ''} flex-shrink-0 transition-transform hover:scale-105`}
    ...
    >
      <img 
        ...
        className={`w-full h-full rounded-full object-cover bg-bg-card ${uploading ? 'opacity-50' : ''}`}
...
export default function App() {
  ...
  const handleRating = async (itemId: string, ratingVal: number, review: string) => {
    ...
    if (userData?.muted) return showToast("You are muted.");
               ...
               <div>
                 ...
                 <label className="flex items-center justify-center w-full h-[46px] bg-[#1a1a1e] border border-white/5 rounded-xl cursor-pointer hover:bg-white/5 transition-colors text-sm text-gray-400">
                    ...
                    <input type="file" onChange={async (e) => {
                        ...
                        if(f) {
                            try {
                                showToast("Uploading PFP...", 1000);
                                const url = await uploadToImgBB(f);
                                setEditProfileForm({...editProfileForm, profilePic: url});
                            } catch (err) {
                                console.error(err);
                                showToast("Failed to upload PFP");
                            }
                        }
                    }} className="hidden"/>
                 </label>
