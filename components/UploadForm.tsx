...
import { X, LayoutGrid, Image as ImageIcon, AlertCircle } from 'lucide-react';
...
export const UploadForm: React.FC<UploadFormProps> = ({ initialData, categories, onClose, onSubmit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(form);
      setIsDirty(false); // Reset dirty state on success so we don't trigger alert
      onClose();
    } catch (error) {
      console.error(error);
  ...
  const handleFile = (e: React.ChangeEvent<HTMLInputElement>, field: 'img' | 'gallery') => {
    ...
    if (field === 'img') {
      ...
      reader.onload = (ev) => setForm(prev => ({ ...prev, img: ev.target?.result as string }));
         ...
         <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                    <label className="flex items-center justify-center w-full h-[46px] bg-[#1a1a1e] border border-white/5 rounded-xl cursor-pointer hover:bg-white/5 transition-colors text-sm text-gray-400">
                      ...
                      {form.img ? "Image Selected" : "Choose Image"}
                      <input type="file" accept="image/*" onChange={e => handleFile(e, 'img')} className="hidden"/>
                    </label>
                </div>
              </div>
            </div>
            ...
            <div>
               ...
               <div className="relative">
                    <label className="flex items-center justify-center w-full h-[46px] bg-[#1a1a1e] border border-white/5 rounded-xl cursor-pointer hover:bg-white/5 transition-colors text-sm text-gray-400">
                      <ImageIcon size={16} className="mr-2"/>
                      Add Screenshots
                      <input type="file" multiple accept="image/*" onChange={e => handleFile(e, 'gallery')} className="hidden"/>
                    </label>
                </div>
               <div className="flex gap-2 mt-3 overflow-x-auto pb-2">
                 {(form.gallery || []).map((src, i) => <img key={i} src={src} className="w-16 h-16 rounded-lg object-cover border border-white/10"/>)}
               </div>
            </div>
            ...
            <div>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <input placeholder="Original Creator Name" value={form.originalCreator || ''} onChange={e => handleChange('originalCreator', e.target.value)} className="w-full bg-[#1a1a1e] border border-white/5 rounded-xl p-3 outline-none focus:border-primary/50 text-white placeholder:text-gray-600 focus:ring-1 focus:ring-primary/50"/>
               </div>
            </div>

            <button disabled={loading} className="w-full bg-primary hover:bg-primary-light py-4 rounded-xl font-bold shadow-lg shadow-purple-900/30 transition-all hover:-translate-y-1 mt-4 text-white disabled:opacity-50">
